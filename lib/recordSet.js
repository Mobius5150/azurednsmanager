...
RecordSet.prototype.AddProperty = function (value) {
	...
	if (this.properties[propName].indexOf(value) !== -1) {
		return;
	}
...
RecordSet.prototype.RemoveProperty = function (value) {
	var propName = RecordTypeData[this.shortType].propertyName;
	...
	for (var p in this.properties[propName]) {
		if (_.isEqual(value, this.properties[propName][p])) {
			this.properties[propName].splice(p, 1);
			return true;
		}
	}
