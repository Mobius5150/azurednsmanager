...
var authProviders = fs.readdirSync(path.join(azCliLibDir, "/util/authentication/"));
var currentAuthProvider = -1;
if (authProviders.length === 0) {
	console.error("No authentication mechanisms available");
	process.exit(1);
}

var auth = null;//require(azCliAdalAuthPath);
...
function loadDefaultProfile(callback) {
	if (++currentAuthProvider >= authProviders.length) {
		cli.error('Could not find valid Azure auth token from Azure CLI. Please run `azure login` and try again.');
		callback(true, null);
		return;
	}

	var provider = require(path.join(azCliLibDir, "/util/authentication/" + authProviders[currentAuthProvider]));
	...
	if (utility.nullOrBadType(provider, 'object') || utility.nullOrBadType(provider.tokenCache, 'object') || utility.nullOrBadType(provider.tokenCache.find, 'function')) {
		loadDefaultProfile(callback);
		return;
	}

	provider.tokenCache.find({ tokenType: 'Bearer', tenantId: subscription.tenantId }, getUserAuthToken);
...
function applyActions(parsedCSVRecords, parsedAzureRecords, actions, callback) {
	...
	for (var path in parsedCSVRecords) {
		for (var type in parsedCSVRecords[path]) {
			...
			if (!utility.nullOrBadType(parsedAzureRecords[path], 'object') && !utility.nullOrBadType(parsedAzureRecords[path][type], 'object')) {
				for (var vKey in parsedAzureRecords[path][type].values) {
					recordSet.AddProperty(parsedAzureRecords[path][type].values[vKey]);
				}
			}
	...
	for (var a in recordActions.remove) {
		...
		if (!utility.nullOrBadType(applySets[path], 'object') && !utility.nullOrBadType(applySets[path][type], 'object')) {
			applySets[set.path][set.type].recordSet.RemoveProperty(set.record);
		}
	}

	for (var path in applySets) {
		for (var type in applySets[path]) {
			if (applySets[path][type].actionCount === 0) {
				continue;
			}
...
module.exports = {
	init: function init(opts, cli, creds) {
		...
		if (typeof options.dnsManager === 'object') {
			dnsManager = options.dnsManager;
		}
