...
function applyActions(parsedCSVRecords, parsedAzureRecords, actions, callback) {
	...
	for (var a in recordSetActions.createAndUpdate) {
		...
		if (typeof applySets[set.path] === 'undefined') {
			applySets[path] = {};
		}

		applySets[path][type] = { recordSet: recordSet, actionCount: 1 };
	}
	...
	for (var a in recordActions.createAndUpdate) {
		...
		if (typeof applySets[set.path][set.type] === 'undefined') {
			applySets[set.path][set.type] = { 
				recordSet: getRecordSetFromParsedRecords(parsedCSVRecords, set.path, set.type),
				actionCount: 0 };
		}
	...
	for (var path in applySets) {
		for (var type in applySets[path]) {
			createOrUpdateRecordSet(applySets[path][type].recordSet, path, type, function handleRecordSetActionComplete(error, result, params) {
				if (error) {
					throw error;
				}
